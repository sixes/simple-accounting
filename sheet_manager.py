...
class SheetManager:
    ...
    def _setup_currency_sheet_structure(self, table, amount_column_title):
        ...
        if table.rowCount() < 102:
            ...
            # Keep existing row count if it's already larger
            pass
    ...
    def populate_aggregate_data(self, table, subject_filter, amount_column_title):
        ...
        if subject_filter in ["销售收入", "销售成本", "银行费用", "利息收入", "董事往来"]:
            ...
            if subject_filter == "董事往来" and hasattr(table, 'user_added_rows'):
                # Save existing user data with original positions before refreshing
                user_data = self.preserve_director_user_data_with_positions(table)
                
                # Ensure minimum rows for user data
            ...
            if subject_filter == "董事往来":
                # Restore user data to original positions instead of after bank data
                self.restore_director_user_data_to_positions(table, user_data)
                
                # Ensure sufficient rows for future user input
                min_rows = max(102, table.rowCount())  # Keep current size or 102 minimum
        ...
        if hasattr(table, 'user_added_rows'):
            # Use the new preserve method to save user data before refreshing
            user_data = self.preserve_director_user_data(table)
        ...
        if hasattr(table, 'user_added_rows'):
            # Use the new restore method for user data
            start_row = len(data_rows)
            self.restore_director_user_data(table, user_data, start_row)
    ...
    def refresh_aggregate_sheet(self, subject_filter, column_title):
        ...
        if current_tab:
            ...
            if subject_filter in ["销售收入", "销售成本", "银行费用", "利息收入", "董事往来"]:
                ...
                user_data = []
                if subject_filter == "董事往來" and hasattr(current_tab, 'user_added_rows'):
            ...
            if hasattr(current_tab, 'user_added_rows') and current_tab.user_added_rows:
                # First scan for any user data that might not be tracked
                self.scan_director_user_data(current_tab)
                # Use the position-preserving method for director sheet
                tab_name = self.main_window.tabs.tabText(self.main_window.tabs.currentIndex())
                if tab_name == "董事往來":
                    user_data = self.preserve_director_user_data_with_positions(current_tab)
                else:
                    user_data = self.preserve_director_user_data(current_tab)
            ...
            if hasattr(current_tab, 'user_added_rows') and user_data:
                # Use the appropriate restore method based on sheet type
                tab_name = self.main_window.tabs.tabText(self.main_window.tabs.currentIndex())
                if tab_name == "董事往來":
                    # For director sheet, restore to original positions
                    self.restore_director_user_data_to_positions(current_tab, user_data)
                    # Ensure sufficient rows for future user input
                    min_rows = max(102, current_tab.rowCount())
                    if current_tab.rowCount() < min_rows:
                        current_tab.setRowCount(min_rows)
                else:
                    # For other sheets, use the old method (position after bank data)
                    bank_data_end = len(data_rows)
                    self.restore_director_user_data(current_tab, user_data, bank_data_end)
                    # Ensure sufficient rows for future user input
                    min_rows = bank_data_end + len(user_data) + 50  # Extra buffer rows
                    if current_tab.rowCount() < min_rows:
                        current_tab.setRowCount(min_rows)
    ...
    def preserve_director_user_data_with_positions(self, table):
        """Preserve user data in director sheet with original row positions"""
        if not hasattr(table, 'user_added_rows') or not table.user_added_rows:
            return []
